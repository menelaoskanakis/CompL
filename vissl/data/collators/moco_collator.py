...
def moco_collator(batch: List[Dict[str, Any]]) -> Dict[str, List[torch.Tensor]]:
    ...
    # TODO: Add more tasks
    for i in ["semseg"]:#, "depth", "edge", "normals"]:
        if i in batch[0]:
            _data = [torch.stack(x[i]) for x in batch]
            output_batch["ss_{}".format(i)] = [torch.stack(_data)[:, 0, :, :]]
            output_batch["ss_{}_momentum".format(i)] = [torch.stack(_data)[:, 1, :, :]]
