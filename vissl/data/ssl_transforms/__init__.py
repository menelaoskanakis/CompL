...
_TRANSFORMS_WITH_LABELS = ["ImgRotatePil", "ShuffleImgPatches", "ImgRotatePilDense"]
_TRANSFORMS_WITH_COPIES = [
    "ImgReplicatePil",
    "ImgReplicatePilWCrop",
    "ImgReplicatePilWHomAdapt",
    "ImgReplicatePilWCropDense",
    "ImgPilToPatchesAndImage",
    "ImgPilToMultiCrop",
]
_TRANSFORMS_WITH_GROUPING = ["ImgPilMultiCropRandomApply"]
_TRANSFORMS_WITH_DENSE_LABELS = ["ImgRotatePilDense", "RandomHorizontalFlipDense", "ToTensorDense",
                                 "CenterCropDense", "RandomScalingDense", "PadImageDense", "RandomCropDense",
                                 "ResizeDense", "ImgReplicatePilWCrop", "ImgReplicatePilWHomAdapt"]
_DENSE_TASKS = ["semseg", "depth", "edge", "normals"]
...
class SSLTransformsWrapper(ClassyTransform):
    ...
    def __init__(self, indices, **args):
        ...
        if self._is_transform_with_dense_labels():
            print("Augmentation {} applied also on dense labels".format(self.name))
        else:
            print("Augmentation {} applied only on input image".format(self.name))
    ...
    def _is_transform_with_dense_labels(self):
        """
        _TRANSFORMS_WITH_DENSE_LABELS = [...]
        """
        if self.name in _TRANSFORMS_WITH_DENSE_LABELS:
            return True
        return False
    ...
    def __call__(self, sample):
        ...
        if self._is_grouping_transform():
            ...
            raise NotImplementedError('Not tested for our dense prediction protocol')
            output = self.transform(sample["data"])
            sample["data"] = output
        else:
            # TODO: Remove comments if everything is working fine
            for idx in indices:
                if self._is_transform_with_dense_labels():
                    input = {'image': sample["data"][idx]}
                    for i in _DENSE_TASKS:
                        if sample.get(i, None) is not None:
                            assert sample["data"][idx].size == sample[i][idx].size
                            input[i] = sample[i][idx]
                    outputs = self.transform(input)
                    output = outputs['image']
                    for key, target in outputs.items():
                        if key != 'image':
                            sample[key][idx] = target
                else:
        ...
        if self._is_transform_with_copies():
            ...
            for i in _DENSE_TASKS:
                if sample.get(i, None) is not None:
                    sample[i] = [val for sublist in sample[i] for val in sublist]
