...
@register_transform("ImgRotatePilDense")
class ImgRotatePilDense(ClassyTransform):
    """
    ...
    def __call__(self, input):
        ...
        #     semseg = TF.rotate(semseg, self.angles[label])

        for key, tensor in input.items():
            if key == 'image':
                input[key] = [TF.rotate(tensor, self.angles[label]), label]
            else:
                input[key] = TF.rotate(tensor, self.angles[label])
        return input

    @classmethod
    def from_config(cls, config: Dict[str, Any]) -> "ImgRotatePilDense":
        """
