...
@register_train_step("depth_semseg_densecl_train_step")
def depth_semseg_densecl_train_step(task):
    """
    ...
    with grad_context, ddp_context, torch_amp_context:
        ...
        # Confirm there are two outputs (since we are doing both depth and rotations)
        ...
        assert len(task.config["TASKS"]) == 3
        assert task.config["TASKS"][2] == "self_supervised"
        # Ensure the index of supervised/self-supervised
        supervised_ind = 0
        supervised_ind_2 = 1
        self_supervised_ind = 2
        ...
        with PerfTimer("loss_compute", perf_stats):
            if task.train:
                supervised_local_loss = task.supervised_loss(model_output[supervised_ind][:batch_size], sample["depth"])
                supervised_local_loss_2 = task.supervised_loss_2(model_output[supervised_ind_2][:batch_size], sample["semseg"])
                self_super_output = []
                for out in model_output[self_supervised_ind]:
                    self_super_output.append(out[batch_size:])
                local_loss = task.loss(self_super_output, sample["target"])
            else:
                supervised_local_loss = task.supervised_loss(model_output[supervised_ind], sample["depth"])
                supervised_local_loss_2 = task.supervised_loss_2(model_output[supervised_ind_2], sample["semseg"])
        ...
        with PerfTimer("loss_all_reduce", perf_stats):
            ...
            supervised_loss_2 = supervised_local_loss_2.detach().clone()
            loss = local_loss.detach().clone()
            task.last_batch.loss = [all_reduce_mean(supervised_loss), all_reduce_mean(supervised_loss_2), all_reduce_mean(loss)]
        ...
        if len(task.supervised_meters_2) > 0 and (
            ...
            with PerfTimer("meters_update", perf_stats):
                if isinstance(model_output[supervised_ind_2], list):
                    if task.train:
                        model_output_cpu = [x.cpu() for x in model_output[supervised_ind_2][:batch_size]]
                    else:
                        model_output_cpu = [x.cpu() for x in model_output[supervised_ind_2]]
                else:
                    if task.train:
                        model_output_cpu = model_output[supervised_ind_2][:batch_size].cpu()
                    else:
                        model_output_cpu = model_output[supervised_ind_2].cpu()
    ...
    if task.train:
        merged_loss = task.config["TASKS_WEIGHTS"][supervised_ind] * supervised_local_loss + \
                      task.config["TASKS_WEIGHTS"][supervised_ind_2] * supervised_local_loss_2 + \
