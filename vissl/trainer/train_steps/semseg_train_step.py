...
@register_train_step("semseg_train_step")
def semseg_train_step(task):
    """
    ...
    with grad_context, ddp_context, torch_amp_context:
        ...
        with PerfTimer("loss_compute", perf_stats):
            local_loss = task.supervised_loss(model_output, semseg)
        ...
        task.losses.append(task.last_batch.loss.data.cpu().item() * semseg.size(0))
        ...
        if len(task.meters) > 0 and (
            ...
            with PerfTimer("meters_update", perf_stats):
                ...
                for meter in task.meters:
                    meter.update(model_output_cpu, semseg.detach().cpu())

        task.last_batch.model_output = model_output
        task.last_batch.target = semseg
