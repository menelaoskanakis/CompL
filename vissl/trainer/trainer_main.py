...
class SelfSupervisionTrainer(object):
    ...
    def train(self):
        ...
        if is_primary():
            ...
            logging.info("Supervised loss is: {}".format(task.supervised_loss))
            logging.info("Supervised loss 2 is: {}".format(task.supervised_loss_2))
            wandb.watch(self.task.base_model, log="all", log_freq=1000)
    ...
    def _advance_phase(self, task: ClassyTask):
        ...
        task.losses = []
        task.supervised_losses = []
        task.supervised_losses_2 = []
