...
from .classification_task.mtl_task import MultiTask, register_task
...
from vissl.supervised_losses import get_loss_functions, key2loss

if is_apex_available():
    import apex


class ContinuousBatchLoading(object):
...
class SelfSupervisionTask(MultiTask):
    """
    ...
    1) converts the model BathNorm layers to the synchronized batchnorm
    ...
    def __init__(self, config: AttrDict):
        ...
        if 'SUPERVISED_LOSS' in self.config:
            self.supervised_losses = []  # set by the hook at start of each epoch or phase
        else:
            self.supervised_losses = None

        if 'SUPERVISED_LOSS_2' in self.config:
            self.supervised_losses_2 = []  # set by the hook at start of each epoch or phase
        else:
            self.supervised_losses_2 = None
    ...
    def build_selfsuper_dataloaders(self, pin_memory: bool) -> torch.utils.data.DataLoader:
        """
        Build PyTorch dataloaders for all the available_splits. We construct the
        standard PyTorch Dataloader and allow setting all dataloader options.
        """
        loader = get_loader(
                dataset=build_dataset(self.config, 'TRAIN_SS'),
                dataset_config=self.config["DATA"]["TRAIN_SS"],
                num_dataloader_workers=self.config.DATA.NUM_DATALOADER_WORKERS,
                pin_memory=pin_memory,
                multi_processing_method=self.config.MULTI_PROCESSING_METHOD,
                device=self.device,
    ...
    def build_selfsuper_test_dataloaders(self, pin_memory: bool) -> torch.utils.data.DataLoader:
        """
        Build PyTorch dataloaders for all the available_splits. We construct the
        standard PyTorch Dataloader and allow setting all dataloader options.
        """
        loader = get_loader(
                dataset=build_dataset(self.config, 'TEST_SS'),
                dataset_config=self.config["DATA"]["TEST_SS"],
                num_dataloader_workers=self.config.DATA.NUM_DATALOADER_WORKERS,
                pin_memory=pin_memory,
                multi_processing_method=self.config.MULTI_PROCESSING_METHOD,
                device=self.device,
    ...
    def _build_supervised_loss(self):
        ...
        the data size parameter here.
        """
        logging.info("Building supervised loss...")
        loss_config = self.config.SUPERVISED_LOSS
        if loss_config['name'] in key2loss:
            loss = get_loss_functions(loss_config['name'], loss_config.get("params", None))
        else:
            raise NotImplementedError("Loss {} has not been implemented yet".format(loss_config['name']))

        return loss

    def _build_supervised_loss_2(self):
        """
        Build the loss used in training. Supports all PyTorch losses
        and custom defined losses.
        ...
        the data size parameter here.
        """
        logging.info("Building supervised loss 2...")
        loss_config = self.config.SUPERVISED_LOSS_2
        if loss_config['name'] in key2loss:
            loss = get_loss_functions(loss_config['name'], loss_config.get("params", None))
        else:
            raise NotImplementedError("Loss {} has not been implemented yet".format(loss_config['name']))
    ...
    def _build_supervised_meters(self):
        """
        Returns meters for task.
        """
        meter_name = self.config["SUPERVISED_METERS"].get("name", "")
        if not meter_name:
            return []
        meter_params = self.config["SUPERVISED_METERS"][meter_name]
        meter_config = {"name": meter_name, **meter_params}
        return [build_meter(meter_config)]

    def _build_supervised_meters_2(self):
        """
        Returns meters for task.
        """
        meter_name = self.config["SUPERVISED_METERS_2"].get("name", "")
        if not meter_name:
            return []
        meter_params = self.config["SUPERVISED_METERS_2"][meter_name]
        meter_config = {"name": meter_name, **meter_params}
        return [build_meter(meter_config)]
    ...
    def _build_model(self):
        ...
        if self.checkpoint_path is None and PathManager.exists(
            ...
            if self.config["MODEL"]["WEIGHTS_INIT"]["PARAMS_FILE"]:
    ...
    def _set_classy_state(self, state):
        ...
        self.supervised_losses = state["supervised_losses"]
    ...
    def prepare(self, pin_memory: bool = False):
        ...
        if "TRAIN_SS" in self.config["DATA"]:
            self.selfsuper_dataloaders = ContinuousBatchLoading(self.build_selfsuper_dataloaders(pin_memory=pin_memory))

        if "TEST_SS" in self.config["DATA"]:
            self.selfsuper_test_dataloaders = ContinuousBatchLoading(self.build_selfsuper_test_dataloaders(pin_memory=pin_memory))
        ...
        if 'SUPERVISED_LOSS' in self.config:
            self.supervised_loss = self._build_supervised_loss()
        else:
            self.supervised_loss = None
        if 'SUPERVISED_LOSS_2' in self.config:
            self.supervised_loss_2 = self._build_supervised_loss_2()
        else:
            self.supervised_loss_2 = None
        self.meters = self._build_meters()
        if 'SUPERVISED_METERS' in self.config:
            self.supervised_meters = self._build_supervised_meters()
        else:
            self.supervised_meters = None
        if 'SUPERVISED_METERS_2' in self.config:
            self.supervised_meters_2 = self._build_supervised_meters_2()
        else:
            self.supervised_meters_2 = None
        ...
        if self.supervised_loss is not None:
            self.supervised_loss = self.supervised_loss.to(self.device)
        if self.supervised_loss_2 is not None:
            self.supervised_loss_2 = self.supervised_loss_2.to(self.device)
        ...
        if self.checkpoint_path is not None:
            ...
            if "supervised_loss" in self.checkpoint:
                self.supervised_loss.load_state_dict(self.checkpoint["supervised_loss"])
                logging.info("======Loaded supervised loss state from checkpoint======")
            if "supervised_loss_2" in self.checkpoint:
                self.supervised_loss_2.load_state_dict(self.checkpoint["supervised_loss_2"])
                logging.info("======Loaded supervised loss 2 state from checkpoint======")
