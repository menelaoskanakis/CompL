    ...
    def on_update(self, task: "tasks.ClassyTask") -> None:
        ...
        if is_primary() and phase_type == "train":
            ...
            if (
                ...
                losses = {}
                if isinstance(task.last_batch.loss, list):
                    assert len(task.config["TASKS"]) == len(task.last_batch.loss)
                    for i, iter_loss in enumerate(task.config["TASKS"]):
                        losses[iter_loss + '_loss'] = round(task.last_batch.loss[i].data.cpu().item(), 5)
                elif isinstance(task.config["TASKS"], list):
                    assert len(task.config["TASKS"]) == 1
                    losses[task.config["TASKS"][0] + '_loss'] = round(task.last_batch.loss.data.cpu().item(), 5)
                else:
                    losses[task.config["TASKS"] + '_loss'] = round(task.last_batch.loss.data.cpu().item(), 5)
                ...
                log_dic = copy.deepcopy(losses)
                log_dic["lr"] = lr_val
                wandb.log(log_dic, step=iteration)
...
class LogLossMetricsCheckpointHook(ClassyHook):
    ...
    def on_forward(self, task: "tasks.ClassyTask") -> None:
        ...
        if isinstance(model_output, list):
            for ind, model_output_item in enumerate(model_output):
                if isinstance(model_output_item, list):
                    has_nan = has_nan or not torch.tensor(
                        [torch.isfinite(x).all() for x in model_output_item]
                    ).all()
                else:
                    has_nan = has_nan or not torch.isfinite(model_output_item).all()
        else:
            has_nan = not torch.isfinite(model_output).all()
        ...
        if task.meters is not None:
            for meter in task.meters:
                if len(task.meters) > 0 and (
                    ...
                    log_dic = {}
                    for key, value in meter_value.items():
                        if key == 'top_1':
                            log_dic[metric_key + '_{}'.format(key)] = value[0]
                        elif key in ['mIoU', 'rmse', 'log_rmse']:
                            log_dic[metric_key + '_{}'.format(key)] = value
                        elif 'IoU' in key:
                            log_dic[metric_key + '_{}'.format(key)] = value
                    ...
                    wandb.log(log_dic, step=task.iteration + 1)
        if task.supervised_meters is not None:
            for meter in task.supervised_meters:
                if len(task.supervised_meters) > 0 and (
                    ...
                    wandb.log(log_dic, step=task.iteration + 1)
        if task.supervised_meters_2 is not None:
