...
from vissl.hooks.densecl_hooks import DenseCLHook  # noqa
        ...
        hooks.extend(
            [
                MoCoHook(
                    ...
                    cfg.LOSS["moco_loss"].get("momentum_enc_copy_params", []),
                    shuffle_batch=False,
                    # shuffle_batch=(not cfg.MODEL.SYNC_BN_CONFIG.CONVERT_BN_TO_SYNC_BN),
                )
            ]
        )
    if cfg.LOSS.name == "densecl_loss":
        hooks.extend(
            [
                DenseCLHook(
                    cfg.LOSS["densecl_loss"]["momentum"],
                    cfg.LOSS["densecl_loss"].get("momentum_enc_copy_params", []),
                    shuffle_batch=False,
                    # shuffle_batch=(not cfg.MODEL.SYNC_BN_CONFIG.CONVERT_BN_TO_SYNC_BN),
                )
            ]
