    ...
    def __init__(self, block, layers, nInputChannels=3, pyramid_pooling="atrous-v3",
        ...
        if decoder:
            ...
            if pyramid_pooling == 'atrous-v3':
                ...
                out_f_pyramid = 256
                self.layer5 = AtrousSpatialPyramidPoolingModule(depth=out_f_pyramid,
                                                                dilation_series=v3_atrous_rates,
                                                                in_f=in_f,
                                                                norm_layers=self.norm_layers)
            else:
                raise NotImplementedError('Select one of the available pooling layers')
            ...
            if norm_layers == 'BatchNorm2d':
                ...
                kwargs_low = {"num_groups": 24, "num_channels": 48, "affine": affine_par}
                kwargs_out = {"num_groups": 32, "num_channels": 256, "affine": affine_par}
            else:
                raise NotImplementedError
...
def get_state_dict(model_name, path=None):
    ...
    elif ('resnet26' in model_name) or model_name in ['resnet50_gn', 'resnet101_gn']:
        if path is None:
...
def resnet50(pretrained=None, nInputChannels=3, path=None,
             norm_layers='BatchNorm2d', **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 4, 6, 3], nInputChannels=nInputChannels, norm_layers=norm_layers, **kwargs)
    if pretrained:
...
def resnet101(pretrained=None, nInputChannels=3, path=None,
             norm_layers='BatchNorm2d', **kwargs):
    """Constructs a ResNet-101 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 4, 23, 3], nInputChannels=nInputChannels, norm_layers=norm_layers, **kwargs)
    if pretrained:
