...
class ResNeXt(nn.Module):
    ...
    def __init__(self, model_config: AttrDict, model_name: str):
        ...
        if self.trunk_config.get("PRETRAINED", False):
            print('Loading resnet{} Imagenet'.format(self.trunk_config.DEPTH))
            state_dict = self.get_state_dict(self.trunk_config.DEPTH)
            try:
                model_conv1.load_state_dict({name: state_dict['conv1.' + name]
                                             for name in model_conv1.state_dict().keys()})
                model_bn1.load_state_dict({name: state_dict['bn1.' + name]
                                           for name in model_bn1.state_dict().keys() if 'num_batches_tracked' not in name})
                model_layer1.load_state_dict({name: state_dict['layer1.' + name]
                                              for name in model_layer1.state_dict().keys() if 'num_batches_tracked' not in name})
                model_layer2.load_state_dict({name: state_dict['layer2.' + name]
                                              for name in model_layer2.state_dict().keys() if 'num_batches_tracked' not in name})
                model_layer3.load_state_dict({name: state_dict['layer3.' + name]
                                              for name in model_layer3.state_dict().keys() if 'num_batches_tracked' not in name})
                model_layer4.load_state_dict({name: state_dict['layer4.' + name]
                                              for name in model_layer4.state_dict().keys() if 'num_batches_tracked' not in name})
            except:
                raise NotImplementedError('Pretraining is currently only supported for base ResNets')
    ...
    def get_state_dict(self, depth):
        # Load checkpoint
        if 'resnet{}'.format(depth) in MODEL_URLS:
            checkpoint = model_zoo.load_url(MODEL_URLS['resnet{}'.format(depth)])
        elif depth == 26:
            if 'PRETRAINED_PATH' not in self.trunk_config:
                raise ValueError('Pretrained model path not specified')
            checkpoint = torch.load(
                os.path.join(self.trunk_config.PRETRAINED_PATH, 'resnet{}.pth'.format(depth)),
                map_location=lambda storage, loc: storage)['state_dict']
        else:
            raise NotImplementedError('Pretraining for resnet{} not available'.format(depth))
